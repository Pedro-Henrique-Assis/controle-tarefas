...
export default function HomeScreen() {
  ...
  // Recarrega os trabalhos sempre que a tela recebe foco
  useFocusEffect(
    useCallback(() => {
      carregarTrabalhos();
    }, [mesSelecionado])
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Scroller horizontal de meses */}
      <FlatList
        data={MESES}
        horizontal
        keyExtractor={(item) => String(item.numero)}
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={styles.scrollerLista}
        renderItem={renderMes}
      />

      <Text style={styles.subtitulo}>
        Trabalhos de {MESES[mesSelecionado - 1].nome}
      </Text>

      {/* Lista de trabalhos filtrada pelo mês */}
      <FlatList
        data={trabalhos}
        keyExtractor={(item) => String(item.ID)}
        contentContainerStyle={styles.listaContainer}
        ListEmptyComponent={
          <Text style={styles.vazio}>Nenhum trabalho com vencimento neste mês.</Text>
        }
...
const styles = StyleSheet.create({
  ...
  titulo: { fontSize: 22, fontWeight: 'bold', color: '#0EA5E9', paddingHorizontal: 20, paddingTop: 20, paddingBottom: 12 },
  scrollerLista: { paddingHorizontal: 16, paddingBottom: 4 },
  mesCard: {
    alignItems: 'center', backgroundColor: '#FFF', borderRadius: 12,
    padding: 12, marginRight: 10, width: 64, borderWidth: 2,
